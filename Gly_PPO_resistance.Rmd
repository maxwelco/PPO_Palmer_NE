...
output: pdf_document
    ...
    keep_tex: yes
  fontsize: 11pt
  geometry: [top=1in, bottom=1in, right=1in, left=1.5in]pdf_document: default
 ---
...
```{r, fig.cap="Regression between the fomesafen molecular and greenhouse studies. Result is presented for run in winter (left) and summer (right) timings.", echo=FALSE, fig.width = 12, fig.height= 6, fig.pos='h'}
ggplot(DataRG, aes(x=Perc, y=PercMolec, label=Population)) + geom_point(aes(size=AvegGA, color=PPO)) + geom_smooth(method='lm',formula=y~x, se=F, color="blue") +
  facet_grid(~Herbicide) + labs(x="% of alive plants 21 days after herbicide application", y="% resistant plants in the molecular screening") + 
...
```{r, echo=FALSE, fig.cap="Regression between the result from the fomesafen molecular and greenhouse study. Result is presented for run in winter (left), summer (center), and fall (right) timings.", echo=FALSE, fig.width = 12, fig.height= 6, fig.pos='h'}
